...
let EtherealMailProvider = exports.default = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async sendMail({
    ...
    const message = await this.client.sendMail({
      from: {
        name: from?.name || 'Equipe LemonadeTechnologies',
        address: from?.email || 'contato@lemonadetechnologies.com.br'
      },
      to: {
        name: to.name,
        address: to.email
